...
const ResultCard = ({ result, fileType }) => {
    ...
    const isAI = typeof rawPrediction === 'number'
        ? rawPrediction === 1
        : typeof rawPrediction === 'string'
            ? rawPrediction.toLowerCase().includes('ai') || rawPrediction.toLowerCase().includes('generated')
            : false;
    ...
    else if (typeof rawConfidence === 'string') numericConfidence = parseFloat(rawConfidence.replace('%', '')) || 0;
    ...
    return (
        <div className={`mt-8 p-8 rounded-3xl border ${isAI ? 'border-red-200 bg-red-50/50' : 'border-green-200 bg-green-50/50'} shadow-inner`}>
            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
                <div className={`p-4 rounded-full ${isAI ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                ...
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Phân tích dựa trên {isVideo ? 'các khung hình video' : 'hình ảnh'} đã tải lên.</p>
                </div>
            </div>
            ...
            <div className="mb-8">
                <div className="flex justify-between items-center mb-2.5">
                    <span className="text-sm font-semibold text-gray-600">Độ tin cậy của mô hình Machine Learning:</span>
                    <span className={`text-xl font-extrabold ${isAI ? 'text-red-600' : 'text-green-600'}`}>{confidence}%</span>
                </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {features.map((feat, index) => (
                        <div key={index} className="bg-white p-4 rounded-xl border border-gray-100 flex items-start gap-3 shadow-sm hover:border-blue-100 transition">
                ...
                <p className="text-xs text-gray-400 mt-5 italic">* Các đặc trưng này được trích xuất bằng thư viện OpenCV và Skimage (FFT, GLCM, v.v.).</p>
            </div>
